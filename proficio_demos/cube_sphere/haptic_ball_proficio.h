...
#ifndef BARRETT_SYSTEMS_HAPTIC_BALL_PROFICIO_H_
#define BARRETT_SYSTEMS_HAPTIC_BALL_PROFICIO_H_
...
class HapticBallProficio : public HapticObject {
	BARRETT_UNITS_FIXED_SIZE_TYPEDEFS;

public:
	HapticBallProficio(const cp_type& center, double radius, const std::string& sysName = "HapticBall") :
		HapticObject(sysName),
		c(center), r(radius), keepOutside(true) /* doesn't matter how this is initialized, it'll fix itself */,
		depth(0.0), error(0.0) {}
	virtual ~HapticBallProficio() { mandatoryCleanUp(); }
...
private:
	DISALLOW_COPY_AND_ASSIGN(HapticBallProficio);
