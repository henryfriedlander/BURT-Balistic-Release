...
def main():
	...
	msg_format = "d" * 6  # messages contain 3 doubles
	...
	#       frame=f)
	target = sphere(pos=center_pos, radius=0.01, color=color.green, opacity=0.6, frame=f)
	...
	while True:
		...
		target.pos = (center_pos[0] + msg[3], center_pos[1] + msg[4], center_pos[2] + msg[5])
		...
		if scene.kb.keys:  # If any input was pressed
			...
			sock.send(struct.pack(msg_format_send, grav_comp, 1, 1, 1, 1))
			sock.send(struct.pack(msg_format_send, 100, 1, 1, 1, 1))
