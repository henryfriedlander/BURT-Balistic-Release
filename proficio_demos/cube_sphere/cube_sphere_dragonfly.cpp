...
// #include "Dragonfly.h" // NOLINT (build/include_subdir)
// #include "message_defs.h" // NOLINT (build/include_subdir)
...
int proficio_main(int argc, char** argv,
  ...
  while (true) {  // Allow the user to stop and resume with pendant buttons
    ...
    if ( counter % 1000 == 0 ) {
		...
		CMessage M( MT_FORCE_FEEDBACK);
		M.SetData( &force_data, sizeof(force_data));
		mod.SendMessageDF( &M);
		
		MDF_POSITION_FEEDBACK pos_data;
		pos_data.x = pos[0];
		pos_data.y = pos[1];
		pos_data.z = pos[2];
		CMessage M( MT_POSITION_FEEDBACK);
		M.SetData( &pos_data, sizeof(pos_data));
		mod.SendMessageDF( &M);		
	}
	counter++;
